...
def save_model(model, epoch: int, parameters: dict, run_path) -> None:
    ...
    if parameters["only save best"]:
        ...
        torch.save(
            model, Path(run_path, "models", "model saved at " + str(epoch) + ".pt")
        )
...
def train_model(
    model, data_loader, device, loss_function, optimizer, scheduler, parameters
):
    ...
    gradient_accumulation_steps = round(
        parameters["desired batch size"] / parameters["real batch size"]
    )
    logger.info(
        f"Using {gradient_accumulation_steps} gradient accumulation steps to achieve a batch size of {parameters['desired batch size']} with batches of {parameters['real batch size']} items."
    )
        ...
        for phase in ["train", "test"]:
            ...
            for index, (inputs, labels) in enumerate(data_loader[phase]):
                ...
                with torch.set_grad_enabled(phase == "train"):
                    ...
                    if phase == "train":
                        loss = (
                            loss_function(outputs, labels) / gradient_accumulation_steps
                        )
                        loss.backward()

                    if phase == "train" and (
                        (index + 1) % gradient_accumulation_steps == 0
                        or (index + 1) == len(data_loader[phase])
                    ):
                        optimizer.step()
                        optimizer.zero_grad()
            ...
            logger.info(
                f"\tphase: {phase}, epoch acc: {round(epoch_accuracy, 2)}, running corrects: {running_corrects}, total checked: {total_count}, current learning rate: {round(scheduler.get_last_lr()[0], 8)}"
            )
            ...
            if phase == "test" and epoch_accuracy > best_accuracy:
                logger.info(
                    f"New best model! Accuracy is {epoch_accuracy}. Saving the model."
                )
...
def prepare_model(data_loaders: dict, parameters: dict) -> None:
    ...
    if parameters["model"] == "efficientnet":
        model = models.efficientnet_v2_s(
            ...
            classes=parameters["number classes"],
        )
    ...
    if parameters["model"] == "convnext":
        ...
        model.classifier[2] = nn.Sequential(
            nn.Linear(768, parameters["number classes"]), nn.Sigmoid()
        )
    ...
    train_model(
        model, data_loaders, device, loss_function, optimizer, scheduler, parameters
    )
