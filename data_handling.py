...
def create_image_folder(
    root: str, split_type: str, batch_size
    ...
    dataset = ImageFolder(
        Path(root, split_type),
        transform=Transforms(transforms=transform_dict[split_type]),
    )

    logger.info(f"For {split_type}, we have {dataset.class_to_idx}")

    data_loader = torch.utils.data.DataLoader(
        dataset, batch_size, shuffle=(split_type == "train"), num_workers=4
    )
